...
function initBuffers() {
  ...
  _binaryImageWrapper = new ImageWrapper(
  ...
  _subImageWrapper = new ImageWrapper(
    _patchSize,
    new Uint8Array(skeletonImageData, 0, _patchSize.x * _patchSize.y)
  );
  _skelImageWrapper = new ImageWrapper(
    _patchSize,
    new Uint8Array(
      skeletonImageData,
  ...
  _patchGrid = new ImageWrapper(
    _imageToPatchGrid.size,
    undefined,
    undefined,
    true
  );
  _patchLabelGrid = new ImageWrapper(
    _imageToPatchGrid.size,
    undefined,
    Int32Array,
    true
...
function similarMoments(moments) {
  const clusters = cluster(moments, 0.9);
    ...
    if (currentIdx < _patchLabelGrid.data.length) {
      ...
      for (dir = 0; dir < Tracer.searchDirections.length; dir++) {
        ...
        if (_patchLabelGrid.data[idx] === 0) {
          similarity = Math.abs(
            vec2.dot(_imageToPatchGrid.data[idx].vec, currentPatch.vec)
          );
...
export default {
  ...
  checkImageConstraints(inputStream, config) {
    ...
    inputStream.setWidth(
      Math.floor(
        Math.floor(size.x / patchSize.x) * (1 / thisHalfSample) * patchSize.x
      )
    );
    inputStream.setHeight(
      Math.floor(
        Math.floor(size.y / patchSize.y) * (1 / thisHalfSample) * patchSize.y
      )
    );
    ...
    throw new Error(
      `Image dimensions do not comply with the current settings: Width (${width} )and height (${height}) must a multiple of ${patchSize.x}`
    );
  },
};
