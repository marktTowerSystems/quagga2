    ...
    function checkVideo(): void {
      if (attempts > 0) {
        if (video.videoWidth > 10 && video.videoHeight > 10) {
          if (ENV.development) {
            console.log(
              `* dev: checkVideo found ${video.videoWidth}px x ${video.videoHeight}px`
            );
          }
          resolve();
        } else {
        ...
        reject(
          new Exception("Unable to play video stream. Is webcam working?", -1)
        ); // TODO: add error code
      }
      attempts--;
    }
...
/**
 * Sets internal ref to existing stream
 * @param {MediaStream} stream
 */
async function setStream(stream: MediaStream): Promise<void> {
  streamRef = stream;
  return Promise.resolve();
}

function deprecatedConstraints(
  videoConstraints: MediaTrackConstraintsWithDeprecated
): MediaTrackConstraints {
  const normalized = pick(videoConstraints, [
    "width",
    "height",
    "facingMode",
    "aspectRatio",
