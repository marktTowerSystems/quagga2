...
const inputStreamFactory: InputStreamFactory = {
  createVideoStream(video): InputStream {
    ...
    function initSize(): void {
      ...
      _calculatedWidth = _config?.size
        ? width / height > 1
          ? _config.size
          : Math.floor((width / height) * _config.size)
        : width;
      // eslint-disable-next-line no-nested-ternary
      _calculatedHeight = _config?.size
        ? width / height > 1
  ...
  createImageStream(): InputStream {
    ...
    function loadImages(): void {
      ...
      ImageLoader.load(
        baseUrl,
        (imgs: Array<{ tags: any; img: HTMLImageElement }>) => {
          imgArray = imgs;
          // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    ...
    var inputStream: InputStream = {
      ...
      clearEventHandlers() {
        Object.keys(_eventHandlers).forEach(
          (ind) => delete _eventHandlers[ind]
        );
      },
