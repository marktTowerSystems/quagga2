...
export default class Quagga {
  ...
  initializeData(imageWrapper?: ImageWrapper): void {
    ...
    this.context.decoder = BarcodeDecoder.create(
  ...
  getBoundingBoxes(): Array<Array<number>> | null {
    return this.context.config?.locate
      ? BarcodeLocator.locate()
      : [
          [
  ...
  transformResult(result: any): void {
    ...
    if (result.barcodes) {
      ...
      result.barcodes.forEach((barcode: BarcodeInfo) =>
        this.transformResult(barcode)
      );
    }
  ...
  addResult(result: QuaggaJSResultObject, imageData: Array<number>): void {
    ...
    if (result.barcodes) {
      result.barcodes
        .filter((barcode: QuaggaJSResultObject) => barcode.codeResult)
        .forEach((barcode: QuaggaJSResultObject) =>
          this.addResult(barcode, imageData)
        );
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  publishResult(
    result: QuaggaJSResultObject | null = null,
    imageData?: any
  ): void {
    let resultToPublish:
      | Array<QuaggaJSResultObject>
      | QuaggaJSResultObject
      | null = result;
  ...
  async locateAndDecode(): Promise<void> {
    ...
    if (boxes) {
      const decodeResult =
        (await this.context.decoder.decodeFromBoundingBoxes(boxes)) || {};
      decodeResult.boxes = boxes;
      this.publishResult(decodeResult, this.context.inputImageWrapper?.data);
    } else {
      const imageResult = await this.context.decoder.decodeFromImage(
        this.context.inputImageWrapper
      );
      if (imageResult) {
  ...
  update = (): void => {
    if (this.context.onUIThread) {
      ...
      if (!workersUpdated) {
        this.context.framegrabber.attachData(
